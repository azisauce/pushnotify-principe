...
webpush.setVapidDetails('mailto:example@app2.com', publicVapidKey, privateVapidKey);
...
app.post('/subscribe', (req, res) => {
  ...
  res.status(201).json({});
  const payload = JSON.stringify({ title: 'App2 Notification', body: 'This is from App2' });
  webpush.sendNotification(subscription, payload).catch(error => console.error(error));
});

io.on('connection', (socket) => {
  console.log('Client connected');
  socket.on('notify-app1', (data) => {
